...
export function SuppliersPanel() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const { data, total: count } = await apiFetchList<Supplier>(`/api/suppliers?q=${encodeURIComponent(q)}`)
      setItems(data)
      setTotal(count)
    } catch {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">Add or update a supplier&#39;s contact details.</DialogDescription>
          </DialogHeader>
