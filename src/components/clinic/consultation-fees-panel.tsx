...
export function ConsultationFeesPanel({ currency = '₹' }: { currency?: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const { data, total: count } = await apiFetchList<Fee>('/api/consultation-fees')
      setItems(data)
      setTotal(count)
    } catch {
  ...
  const remove = async () => {
    if (!deleteId) return
    try {
      await apiFetch(`/api/consultation-fees/${deleteId}`, { method: 'DELETE' })
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">Set the name and amount of a consultation fee.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
