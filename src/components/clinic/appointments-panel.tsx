...
export function AppointmentsPanel({ currency = '₹' }: { currency?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data, total: count } = await apiFetchList<Appointment>(
  ...
  useEffect(() => {
    apiFetch<Staff[]>('/api/staff?role=Doctor').then(setDoctors).catch(() => {})
    apiFetch<Fee[]>('/api/consultation-fees').then(setFees).catch(() => {})
  }, [])
  ...
  const remove = async () => {
    if (!deleteId) return
    try {
      await apiFetch(`/api/appointments/${deleteId}`, { method: 'DELETE' })
      toast({ title: 'Appointment cancelled', description: 'The record is kept for history' })
      setDeleteId(null)
      load()
    } catch (err) {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">Schedule a patient visit with an optional consultation fee.</DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              The appointment is marked Cancelled and kept in the records — visit and
