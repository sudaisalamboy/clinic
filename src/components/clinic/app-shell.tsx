...
export function AppShell({
  ...
  const fetchStockAlerts = useCallback(() => {
    apiFetch<StockAlert[]>('/api/inventory/items?filter=low')
      .then((d) => {
        // Items at or below their minimum stock threshold.
        // Matches the server's `quantity <= minStock` filter used by the
        // dashboard low-stock list and the reports lowStockItems query.
        const alerts = (Array.isArray(d) ? d : []).filter((item) =>
    ...
    <div className="h-screen flex overflow-hidden bg-muted/20">
      ...
      >
        ...
        <div className="h-14 flex items-center gap-2.5 px-3 border-b border-sidebar-border shrink-0">
          ...
          >
            {settings?.logo ? (
              // Admin-uploaded logo: a runtime data-URL or arbitrary remote
              // URL, which next/image cannot optimize without a remotePatterns
              // allowlist — plain <img> is deliberate here.
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-6">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {active === 'billing' && <BillingPanel currency={currency} role={user.role} />}
              {active === 'inventory' && <InventoryPanel currency={currency} role={user.role} />}
