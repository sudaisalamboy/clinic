...
interface ReportData {
  ...
  expiringItems: ExpiringReportItem[]
}
...
export function ReportsPanel({ currency = '₹' }: { currency?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const d = await apiFetch<ReportData>(`/api/reports?${params.toString()}`, { cache: 'no-store' })
      setData(d)
    } catch {
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        cy="50%"
                        outerRadius={90}
                        label={(entry: { status?: string; count?: number }) => `${entry.status ?? ''}: ${entry.count ?? 0}`}
                      >
